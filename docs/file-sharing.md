...
#### Multi-Source/Peer Download Details:
- Architecture:
  - Files are split into pieces of equal length (except for the last piece). When one peer downloads a piece of the file they need, it informs the rest of the peers that “I have this piece” so that the rest of the peers can start downloading from this peer who now currently has this piece. In the same way, every other peer does the same thing
...
- Problems: 
  ...
  - How will the rest of the peers get the other pieces? This means that we need an algorithm that distributes the other (the ones we are missing) pieces to some other random peer so that we don’t have to worry about whether the seeder shuts down after piece N
- Solution: 
...
#### Strategies
- Rarest-First Piece Selection: prioritize the download of the piece that is the rarest in the network
  - Rarest-first piece selection strategy ensures that only the ‘unchoked’ pieces (the piece that is currently not being downloaded/leeched) are downloaded first
  - This leads to quick distribution of pieces in the network and a reduced load on the seeder 
  - Increased download speed since when a peer has a rare piece, every other peer would be interested in downloading it. Because of reciprocation, it would be frequently unchoked from others, allowing for better download speeds
