use chiral_network::stream_auth::{StreamAuthService, AuthMessageType};
...
async fn test_hmac_authentication_for_unencrypted_transfers() {
    ...
    assert!(auth_service.create_session(session_id.to_string(), hmac_key).is_ok());
    ...
    let auth_msg = auth_service.create_authenticated_chunk(
        session_id,
        chunk_data,
        chunk_index,
        file_hash,
    ...
    // Test verification
    let verification_result = auth_service.verify_data(session_id, &auth_msg)
        .expect("Failed to verify data");
    assert!(verification_result);
    
    // Test extracting chunk data
    let extracted_data = auth_service.verify_authenticated_chunk(session_id, &auth_msg)
...
async fn test_hmac_key_exchange_flow() {
    ...
    let request = auth_service.initiate_key_exchange(
    ...
    let response = auth_service.respond_to_key_exchange(
        request,
    ...
    let confirmation = auth_service.confirm_key_exchange(
        response,
        initiator_peer_id.clone(),
    ...
    auth_service.finalize_key_exchange(
        confirmation,
        responder_peer_id.clone(),
    ).expect("Failed to finalize key exchange");
    
    // Verify the session was created with the exchanged key
    let session_info = auth_service.get_session_info(&format!("{}-{}", initiator_peer_id, session_id));
...
async fn test_encrypted_vs_unencrypted_authentication() {
    ...
    let verification = auth_service.verify_data(session_id, &auth_msg)
        .expect("Failed to verify data");
    assert!(verification);
