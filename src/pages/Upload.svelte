  ...
  async function showVersionHistory(fileName: string) {
    try {
      ...
      if (versions && versions.length > 0) {
        ...
        const versionInfo = sortedVersions.map(v => 
          `v${v.version} (${new Date(v.created_at * 1000).toLocaleDateString()})`
        ).join(', ');
        showToast(`${fileName} versions: ${versionInfo}`, 'info');
      } else {
        showToast(`No version history found for ${fileName}`, 'warning');
      }
    } catch (error) {
      showToast(`Could not load version history: ${error}`, 'error');
    }
  }
