<script lang="ts">
  ...
  // UI variables
  const nodeAddress =
    "enode://277ac35977fc0a230e3ca4ccbf6df6da486fd2af9c129925b1193b25da6f013a301788fceed458f03c6c0d289dfcbf7a7ca5c0aef34b680fcbbc8c2ef79c0f71@127.0.0.1:30303";
  ...
  async function startDht() {
    ...
    try {
      ...
      if (backendPeerId) {
        ...
        console.log(
          "DHT already running in backend with peer ID:",
          backendPeerId,
        );
        ...
        dhtEvents = [
          ...dhtEvents,
          `✓ DHT already running with peer ID: ${backendPeerId.slice(0, 16)}...`,
        ];
        ...
        if (peerCount > 0) {
          ...
          dhtEvents = [
            ...dhtEvents,
            `⚠ No peers connected, attempting to connect to bootstrap...`,
          ];
        ...
        try {
          ...
          dhtEvents = [
            ...dhtEvents,
            `✓ DHT started with peer ID: ${peerId.slice(0, 16)}...`,
          ];
        } catch (error: any) {
          if (error.toString().includes("already running")) {
            ...
            console.warn(
              "DHT already running in backend, attempting to retrieve peer ID...",
            );
            dhtEvents = [
              ...dhtEvents,
              `⚠ DHT already running in backend, retrieving peer ID...`,
            ];
            ...
            try {
              ...
              if (peerId) {
                ...
                dhtEvents = [
                  ...dhtEvents,
                  `✓ Retrieved peer ID: ${dhtPeerId.slice(0, 16)}...`,
                ];
              } else {
      ...
      if (dhtBootstrapNode) {
        ...
        dhtEvents = [
          ...dhtEvents,
          `[Attempt ${connectionAttempts}] Connecting to ${dhtBootstrapNode}...`,
        ];
        ...
        try {
          ...
          dhtEvents = [
            ...dhtEvents,
            `✓ Connection initiated to bootstrap node (waiting for handshake...)`,
          ];

          // Poll for actual connection after a delay
          setTimeout(async () => {
            const dhtPeerCountResult = (await invoke(
              "get_dht_peer_count",
            )) as number;
            if (dhtPeerCountResult > 0) {
              dhtEvents = [
                ...dhtEvents,
                `✓ Successfully connected! Peers: ${dhtPeerCountResult}`,
              ];
            } else {
              dhtEvents = [
                ...dhtEvents,
                `⚠ Connection pending... (bootstrap node may be unreachable)`,
              ];
            }
          }, 3000);
        } catch (error: any) {
          ...
          if (errorMessage.includes("DHT not started")) {
            errorMessage =
              "DHT service not initialized properly. Try stopping and restarting.";
          } else if (errorMessage.includes("DHT networking not implemented")) {
            errorMessage =
              "P2P networking not available (requires libp2p implementation)";
            ...
            errorMessage =
              "Connection timed out - bootstrap node may be unreachable";
          }
  ...
  function runDiscovery() {
    ...
    setTimeout(() => {
      const newPeer = {
        ...
        joinDate: new Date(),
        lastSeen: new Date(),
        location: ["US-East", "EU-West", "Asia-Pacific", "US-West"][
  ...
  async function downloadGeth() {
    if (!isTauri) {
      downloadError =
        "Chiral Node download is only available in the desktop app. Please download and run the Tauri desktop version.";
      return;
    }
  ...
  async function startGethNode() {
    if (!isTauri) {
      console.log(
        "Cannot start Chiral Node in web mode - desktop app required",
      );
      return;
    }
...
<div class="space-y-6">
  <div>
    ...
    <p class="text-muted-foreground mt-2">
      Monitor network health and discover peers
    </p>
  </div>
  ...
  <Card class="p-6">
    ...
    <div class="space-y-3">
        ...
        {#if isDownloading}
          <div class="space-y-3">
            <div class="text-center py-2">
              <Download
                class="h-12 w-12 text-blue-500 mx-auto mb-2 animate-pulse"
              />
        ...
        {:else}
          <div class="text-center py-4">
            ...
            <p class="text-sm text-muted-foreground mb-1">
              Chiral node binary not found
            </p>
            <p class="text-xs text-muted-foreground mb-3">
              Download the Core-Geth binary to run a local node
            </p>
            {#if downloadError}
              <div
                class="bg-red-500/10 border border-red-500/20 rounded-lg p-2 mb-3"
              >
      ...
      {:else if isStartingNode}
        <div class="text-center py-4">
          <Server
            class="h-12 w-12 text-yellow-500 mx-auto mb-2 animate-pulse"
          />
    ...
    <div class="space-y-3">
      {#if dhtStatus === "disconnected"}
        <div class="text-center py-4">
          ...
          {#if dhtError}
            <div
              class="bg-red-500/10 border border-red-500/20 rounded-lg p-3 mb-3 mx-4"
            >
          ...
          {#if dhtEvents.length > 0}
            <div class="mt-4 mx-4">
              ...
              <div
                class="bg-secondary/50 rounded-lg p-2 max-h-32 overflow-y-auto text-left"
              >
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold">
            {dhtStatus === "connected" ? dhtPeerCount : 0}
          </p>
        </div>
      </div>
    </Card>
    ...
    <Card class="p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold">
            {formatSize($networkStats.networkSize)}
          </p>
        </div>
      </div>
    </Card>
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm font-bold">
            ↓ {$networkStats.avgDownloadSpeed.toFixed(1)} MB/s
          </p>
          <p class="text-sm font-bold">
            ↑ {$networkStats.avgUploadSpeed.toFixed(1)} MB/s
          </p>
        </div>
      </div>
    </Card>
  ...
  <Card class="p-6">
    <div class="flex flex-wrap items-center justify-between gap-2 mb-4">
      ...
      <div class="flex-shrink-0">
        ...
        >
          <RefreshCw
            class="h-4 w-4 mr-2 {discoveryRunning ? 'animate-spin' : ''}"
          />
    ...
    <div class="space-y-3">
        ...
        switch (sortBy) {
          ...
          case "location":
            ...
            const getLocationDistance = (peerLocation: string | undefined) => {
              ...
              const distanceMap: Record<string, Record<string, number>> = { "US-East": { "US-East": 0, "US-West": 1, "EU-West": 2, "Asia-Pacific": 3 }, "US-West": { "US-West": 0, "US-East": 1, "EU-West": 3, "Asia-Pacific": 2 }, "EU-West": { "EU-West": 0, "US-East": 1, "US-West": 3, "Asia-Pacific": 2 }, "Asia-Pacific": { "Asia-Pacific": 0, "US-West": 1, "EU-West": 2, "US-East": 3 } };
        ...
        <div class="p-4 bg-secondary rounded-lg">
          ...
          >
            <div class="flex items-start gap-3 min-w-0">
              <div
                class="w-2 h-2 rounded-full flex-shrink-0 {peer.status ===
                'online'
                  ? 'bg-green-500'
                  : peer.status === 'away'
                    ? 'bg-yellow-500'
          ...
          <div class="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
            ...
            <div>
              ...
              <p class="font-medium">
                {new Date(peer.joinDate).toLocaleString()}
              </p>
            </div>
            <div>
      ...
      {#if $peers.length === 0}
        <p class="text-center text-muted-foreground py-8">
          No peers connected. Run discovery to find peers.
        </p>
      {/if}
    </div>
  </Card>
