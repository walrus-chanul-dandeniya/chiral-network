...
<div class="relative mt-2" bind:this={container}>
  <button
    type="button"
    id={id}
    class="w-full px-3 py-2 border rounded-md bg-white text-left flex items-center justify-between hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
    on:click={() => (open = !open)}
    disabled={disabled}
  >
