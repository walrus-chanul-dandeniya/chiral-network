...
export function generateEntropy(strength: 128 | 160 | 192 | 224 | 256 = 128): Uint8Array {
...
export async function mnemonicToEntropy(mnemonic: string): Promise<Uint8Array> {
  ...
  const newChecksum = bytesToBinary(await sha256(entropy)).slice(0, checksumBits.length)
...
export async function validateMnemonic(m: string): Promise<boolean> {
  try { await mnemonicToEntropy(m); return true } catch { return false }
}
...
export async function mnemonicToSeed(mnemonic: string, passphrase = ''): Promise<Uint8Array> {
  ...
  const bits = await crypto.subtle.deriveBits({ name: 'PBKDF2', hash: 'SHA-512', iterations: 2048, salt }, key, 64 * 8)
