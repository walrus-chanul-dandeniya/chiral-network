...
export class SignalingService {
  ...
  private outQueue: Array<{ msg: any; opts?: { prefer?: 'dht' | 'ws' | 'auto' } }> = [];
    ...
    if (!this.dhtService) {
      try {
        ...
        if (core && typeof (core as any).invoke === 'function') {
          const id = await (core as any).invoke('get_dht_peer_id').catch(() => null);
          if (id) {
  ...
  private persistPeers() {
    ...
    try {
      const arr = Array.from(this.peersMap.entries()).map(([id, ts]) => ({ id, ts }));
  ...
  private loadPersistedPeers() {
    try {
      ...
      for (const p of arr) {
        // skip stale entries
        if (now - (p.ts ?? 0) <= this.peerTtlMs) this.peersMap.set(p.id, p.ts ?? now);
      }
  ...
  private async connectDht(): Promise<void> {
    ...
    try {
      if (this.dhtService) {
        if (typeof this.dhtService.on === 'function') {
          try {
            this.dhtService.on('peers', (list: any[]) => {
              const ids = (list || []).map((p: any) => (typeof p === 'string' ? p : p.peerId || p.id || p.clientId));
              this.mergePeers(ids);
            });
          } catch (_) { /* ignore */ }
        }
        ...
        if (typeof this.dhtService.on === 'function' && typeof (this.dhtService as any).on === 'function') {
          try {
            (this.dhtService as any).on('signal', (msg: any) => this.handleIncoming(msg));
          } catch (_) { /* ignore */ }
        }
      }
      ...
      try {
        ...
        if (evt && typeof (evt as any).listen === 'function') {
          (evt as any).listen('dht_peer_update', (e: any) => {
            const payload = e.payload;
            if (payload && Array.isArray(payload.peers)) this.mergePeers(payload.peers);
          }).catch(() => {});
        }
      } catch (_) { /* ignore */ }
  ...
  private async connectWebSocket(): Promise<void> {
    ...
    try {
      ...
      this.ws.onopen = () => {
        ...
        this.connected.set(true);
        // register
        try { this.ws?.send(JSON.stringify({ type: 'register', clientId: this.clientId })); } catch (_) {}
      ...
      this.ws.onmessage = (ev: MessageEvent) => {
        try {
          ...
          if (data && data.type === 'ping') {
            try { this.ws?.send(JSON.stringify({ type: 'pong', ts: Date.now(), from: this.clientId })); } catch (_) {}
            return;
          }
        ...
        } catch (e) { console.warn('[SignalingService] invalid ws message', e); }
      };

      this.ws.onclose = () => {
        if (this.wsClosedByUser) { this.state.set('disconnected'); this.connected.set(false); this.backend.set('none'); this.stopWsHeartbeat(); return; }
        this.state.set('disconnected');
        this.connected.set(false);
      ...
      this.ws.onerror = (e) => { console.error('[SignalingService] WS error', e); };
    } catch (e) {
  ...
  private scheduleWsReconnect() {
    ...
    const delay = Math.min(this.wsReconnectBase * (2 ** (this.wsReconnectAttempts - 1)), this.wsReconnectMax);
  ...
  private startWsHeartbeat() {
    ...
    this.wsHeartbeatTimer = setInterval(() => {
      try { this.ws?.send(JSON.stringify({ type: 'ping', ts: Date.now(), from: this.clientId })); } catch (e) { /* ignore */ }
    }, this.wsHeartbeatIntervalMs);
  }

  private stopWsHeartbeat() {
    try { if (this.wsHeartbeatTimer) clearInterval(this.wsHeartbeatTimer); } catch (_) {}
  ...
  private async flushQueue() {
    ...
    for (const item of queue) {
      try {
        ...
        console.warn('[SignalingService] failed to flush queued message, re-enqueueing', e);
  ...
  /* ----------------------
     Public API
     ---------------------- */
  setOnMessage(handler: (msg: any) => void) { this.onMessageHandler = handler; }
  ...
  async send(msg: any, opts?: { prefer?: 'dht' | 'ws' | 'auto' }) {
    ...
    if (prefer === 'dht' || (prefer === 'auto' && this.dhtAvailable && this.preferDht)) {
      try { await this.sendViaDht(msg); return; } catch (e) { console.warn('sendViaDht failed, falling back', e); }
    }

    // If WS is ready, send
    if (isWsReady) {
      try { await this.sendViaWs(msg); return; } catch (e) { console.warn('sendViaWs failed', e); }
    }

    // If WS not ready but we can connect, attempt to connect and queue
    if (!isWsReady && currentBackend !== 'ws') {
      try { this.connectWebSocket().catch(() => {}); } catch (_) {}
    }

    // If DHT is available and prefer is ws fallback, try DHT as last resort
    if (this.dhtAvailable && (prefer === 'ws' || prefer === 'auto')) {
      try { await this.sendViaDht(msg); return; } catch (e) { /* ignore */ }
    }
  ...
  disconnect(): void {
    ...
    this.state.set('disconnected');
    this.stopWsHeartbeat();
    try { if (this.peerGcTimer) clearInterval(this.peerGcTimer); } catch (_) {}
  }
  ...
  private handleIncoming(message: any) {
    ...
    if (message.type === 'peers' && Array.isArray(message.peers)) {
      const ids = message.peers.map((p: any) => (typeof p === 'string' ? p : p.clientId || p.id || p.peerId));
      this.mergePeers(ids);
      return;
    }
    ...
    if (message.type === 'offer' || message.type === 'answer' || message.type === 'candidate') {
      if (this.onMessageHandler) {
        try { this.onMessageHandler(message); } catch (e) { console.error('onMessageHandler error', e); }
        return;
      }
    }
