...
export async function saveSeedList(seeds: SeedRecord[]): Promise<void> {
  const payload = JSON.stringify({ version: 1, seeds })
  try {
    ...
    console.warn('tauri write_seed_list failed, falling back to localStorage', e)
  }
...
export async function loadSeedList(): Promise<SeedRecord[]> {
  try {
    ...
    console.warn('tauri read_seed_list failed, falling back to localStorage', e)
  }
  ...
  try {
    ...
    console.error('Failed to parse persisted seed list; ignoring and returning empty list', e)
  }
  return []
}
